...
function dec2hexFormat($strIn, $nrChars) {
   ...
   if (strlen($strHexa) != $nrChars) throw new Exception("Number of Hexa Chars does not match!");
   return $strHexa;
}
...
function genFrankierID($arrShipPack) {
   ...
   $strStampID  .= $strCRC4;
   return substr($strStampID, 0, 2) . ' ' . substr($strStampID, 2, 4) . ' ' . substr($strStampID, 6, 4) . ' ' . substr($strStampID, 10, 2) . ' ' .
